...
int isEpsilon[99500];
int createfirst(char* term){//returns 0 if first contains epsilon,otherwise returns 1
    //printf("%s",term);
    if(strlen(ntFirst[get_hash(term)])){
        // printf("%s",term);
        return isEpsilon[get_hash(term)];
    }
    printf("%d:%s",isupper(term[0]),term);
    if(isupper(term[0])){       //if term is a terminal
        if(strcmp(term,"EPSILON")){     //and its not an epsilon
            if(!strlen(ntFirst[get_hash(term)]))  //if ntFirst of that term isnt empty  
                strcat(ntFirst[get_hash(term)],term);   //append , and term
    ...
    for(int i=0;i<128;i++){  
        if(!strcmp(grammar[i]->value,term)){  //matching nt found
            struct node* temp = grammar[i]->link;     //LHS
            while(temp!=NULL && createfirst(temp->value)){       //if temp is a terminal (except epsilon) or a nt (which doesnt derive epsilon)
...
int main()
{
    ...
    //     printf("%s:%s\n",nonTerminals[i],ntFirst[get_hash(nonTerminals[i])]);
     //printf("%s",ntFirst[get_hash("ioStmt")]);
    return 0;
}